// ================================================================
  ...
  async function uploadToCloudinaryFile(file, resourceType){
  ...
  async function uploadToCloudinary(file){
    return uploadToCloudinaryFile(file, 'video');
  }
  ...
  async function loadIntroVideo(){
    ...
    if(!shown){
      box.innerHTML = `<div class="play">▶</div><p>سيتم إضافة فيديو تعريفي بالنشاط قريباً</p>`;
    }
  ...
  function renderIntroAdminControls(box){
    ...
    controls.style.cssText = 'text-align:center; margin-top:10px; display:flex; gap:10px; justify-content:center; flex-wrap:wrap;';
    controls.innerHTML = `
      <label style="cursor:pointer; color:var(--gold); font-size:.85rem; border:1px solid var(--gold); border-radius:6px; padding:6px 12px;">
        <span class="upload-label">📤 رفع / استبدال الفيديو</span>
        <input type="file" accept="video/*" style="display:none">
      </label>
      <button id="introDeleteBtn" style="cursor:pointer; color:#c0524a; background:none; border:1px solid #c0524a; border-radius:6px; padding:6px 12px; font-family:'Tajawal',sans-serif; font-size:.85rem;">🗑 حذف الفيديو</button>
    `;
    controls.querySelector('input').addEventListener('change', handleIntroUpload);
    controls.querySelector('#introDeleteBtn').addEventListener('click', async () => {
      if(!confirm('حذف فيديو النشاط؟ لا يمكن التراجع.')) return;
      await fetch(`${FIREBASE_DB}/introVideo.json`, { method: 'DELETE' });
      loadIntroVideo();
    });
    box.after(controls);
  }
  ...
  // ===== الروابط السريعة — يديرها الأدمن من الموقع، أو تبويب "الروابط": التسمية | الرابط =====
  async function renderQuickLinks(){
    const targets = { quarterly:'link-quarterly', weekly:'link-weekly', records:'link-records', radio:'link-radio' };
    let liveData = null;
    try{
      const res = await fetch(`${FIREBASE_DB}/quickLinks.json`);
      liveData = await res.json();
    }catch(e){ /* تعذر الاتصال بقاعدة البيانات */ }
    if(liveData){
      Object.keys(targets).forEach(key => {
        const el = document.getElementById(targets[key]);
        if(el && liveData[key]) el.href = liveData[key];
      });
    }else{
      try{
    ...
    if(isAdmin) renderQuickLinksAdmin(targets);
  }

  function renderQuickLinksAdmin(targets){
    Object.keys(targets).forEach(key => {
      const el = document.getElementById(targets[key]);
      if(!el || el.parentElement.querySelector('.ql-edit')) return;
      const btn = document.createElement('button');
      btn.className = 'ql-edit';
      btn.textContent = '✏️ تعديل الرابط';
      btn.addEventListener('click', async (e) => {
        e.preventDefault();
        const url = prompt('الصقي رابط الملف هنا:', el.href && el.href !== '#' && !el.href.endsWith('/#') ? el.href : '');
        if(!url) return;
        await fetch(`${FIREBASE_DB}/quickLinks/${key}.json`, { method: 'PUT', body: JSON.stringify(url) });
        el.href = url;
      });
  ...
  // ===== ملفات مهمة — يديرها الأدمن من الموقع، أو تبويب "الملفات": الاسم | الرابط =====
  async function renderFiles(){
    const list = document.getElementById('filesList');
    let gotLive = false;
    try{
      const res = await fetch(`${FIREBASE_DB}/importantFiles.json`);
      const data = await res.json();
      if(data){
        const entries = Object.entries(data).reverse();
        list.innerHTML = entries.map(([id, f]) => `
          <li>${esc(f.name)} <a href="${f.url}" target="_blank" rel="noopener">تحميل</a>${isAdmin ? ` <button class="f-delete" data-id="${esc(id)}">🗑</button>` : ''}</li>
        `).join('');
        gotLive = true;
        if(isAdmin){
          list.querySelectorAll('.f-delete').forEach(btn => {
            btn.addEventListener('click', async () => {
              if(!confirm('حذف هذا الملف؟ لا يمكن التراجع.')) return;
              await fetch(`${FIREBASE_DB}/importantFiles/${btn.dataset.id}.json`, { method: 'DELETE' });
              renderFiles();
            });
          });
        }
    ...
    if(isAdmin) renderFilesUploadBox(list);
  }

  function renderFilesUploadBox(list){
    const old = document.getElementById('filesUploadRow');
    if(old) old.remove();
    const li = document.createElement('li');
    li.id = 'filesUploadRow';
    li.innerHTML = `<label style="cursor:pointer; color:var(--gold);">
      <span class="upload-label">+ إضافة ملف</span>
      <input type="file" style="display:none">
    </label>`;
    li.querySelector('input').addEventListener('change', handleFileUpload);
    list.prepend(li);
  }
  ...
  async function handleFileUpload(e){
    const file = e.target.files[0];
    if(!file) return;
    const name = prompt('اسم الملف:', file.name) || file.name;
    const labelText = e.target.closest('label').querySelector('.upload-label');
    labelText.textContent = 'جاري الرفع...';
    try{
      const url = await uploadToCloudinaryFile(file, 'raw');
      await fetch(`${FIREBASE_DB}/importantFiles.json`, {
        method: 'POST',
        body: JSON.stringify({ name, url, time: Date.now() })
      });
      renderFiles();
    }catch(err){
      alert('تعذر رفع الملف، حاولي مرة أخرى.');
      labelText.textContent = '+ إضافة ملف';
    }
  }
  ...
  // ===== إعلانات الأنشطة — يديرها الأدمن من الموقع، أو تبويب "الإعلانات": العنوان | التاريخ | الصورة (اختياري) =====
  async function renderAnnouncements(){
    const box = document.getElementById('announceList');
    let gotLive = false;
    try{
      const res = await fetch(`${FIREBASE_DB}/announcementsLive.json`);
      const data = await res.json();
      if(data){
        const entries = Object.entries(data).reverse();
        box.innerHTML = entries.map(([id, a]) => `
          <div class="announce">
            <div style="flex:1;">
              ...
              <p>${esc(a.title)}</p>
              ${isAdmin ? `<button class="an-delete" data-id="${esc(id)}">🗑 حذف</button>` : ''}
            </div>
          </div>`).join('');
        gotLive = true;
        if(isAdmin){
          box.querySelectorAll('.an-delete').forEach(btn => {
            btn.addEventListener('click', async () => {
              if(!confirm('حذف هذا الإعلان؟ لا يمكن التراجع.')) return;
              await fetch(`${FIREBASE_DB}/announcementsLive/${btn.dataset.id}.json`, { method: 'DELETE' });
              renderAnnouncements();
            });
          });
        }
    ...
    if(isAdmin) renderAnnounceUploadBox(box);
  }

  function renderAnnounceUploadBox(box){
    const old = document.getElementById('announceAddRow');
    if(old) old.remove();
    const div = document.createElement('div');
    div.id = 'announceAddRow';
    div.className = 'announce';
    div.style.cssText = 'justify-content:center; cursor:pointer; border:2px dashed var(--gold); background:rgba(199,154,75,.08);';
    div.innerHTML = `<span style="color:var(--green-deep); font-weight:700;">+ إضافة إعلان</span>`;
    div.addEventListener('click', handleAddAnnouncement);
    box.prepend(div);
  }

  async function handleAddAnnouncement(){
    const title = prompt('نص الإعلان:');
    if(!title) return;
    const date = prompt('التاريخ (مثال: هذا الأسبوع):', '') || '';
    await fetch(`${FIREBASE_DB}/announcementsLive.json`, {
      method: 'POST',
      body: JSON.stringify({ title, date, time: Date.now() })
    });
    renderAnnouncements();
  }
  ...
  // ===== الطالبات المتميزات — يديرها الأدمن من الموقع، أو تبويب "الطالبات": الاسم | الصف | الإنجاز =====
  async function renderStudents(){
    const grid = document.getElementById('studentsGrid');
    let gotLive = false;
    try{
      const res = await fetch(`${FIREBASE_DB}/studentsLive.json`);
      const data = await res.json();
      if(data){
        const entries = Object.entries(data).reverse();
        grid.innerHTML = entries.map(([id, s]) => {
          const del = isAdmin ? `<button class="st-delete" data-id="${esc(id)}">🗑 حذف</button>` : '';
          return `
          <div class="empty-card" style="border-style:solid;">
            <div class="ic">★</div>
            <div style="font-weight:700; color:var(--green-deep); margin-bottom:4px;">${esc(s.name)}</div>
            <div style="font-size:.85rem;">${esc(s.grade)}</div>
            <div style="font-size:.82rem; margin-top:6px;">${esc(s.achievement)}</div>
            ${del}
          </div>`;
        }).join('');
        gotLive = true;
        if(isAdmin){
          grid.querySelectorAll('.st-delete').forEach(btn => {
            btn.addEventListener('click', async () => {
              if(!confirm('حذف هذه الطالبة من القائمة؟')) return;
              await fetch(`${FIREBASE_DB}/studentsLive/${btn.dataset.id}.json`, { method: 'DELETE' });
              renderStudents();
            });
          });
        }
    ...
    if(isAdmin) renderStudentsUploadBox(grid);
  }

  function renderStudentsUploadBox(grid){
    const old = document.getElementById('studentAddCard');
    if(old) old.remove();
    const card = document.createElement('div');
    card.id = 'studentAddCard';
    card.className = 'empty-card';
    card.style.cssText = 'cursor:pointer; border-style:dashed;';
    card.innerHTML = `<div class="ic">+</div><div>إضافة طالبة</div>`;
    card.addEventListener('click', handleAddStudent);
    grid.prepend(card);
  }

  async function handleAddStudent(){
    const name = prompt('اسم الطالبة:');
    if(!name) return;
    const grade = prompt('الصف:', '') || '';
    const achievement = prompt('الإنجاز:', '') || '';
    await fetch(`${FIREBASE_DB}/studentsLive.json`, {
      method: 'POST',
      body: JSON.stringify({ name, grade, achievement, time: Date.now() })
    });
    renderStudents();
  }
